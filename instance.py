...
class Instance():
    ...
    def __init__(self,
        ...
        self.model_memory = self.model.size.total_size
        ...
        self.max_available_memory = self.max_memory - self.model.size.total_size

        ## task queues
        self.pending_queue = SortedList(key=lambda x: x.get_id())
        self.blocked_queue = []
        self.batch = []
        ...
        ## instance logger
        # if self.debug:
        #     logger_name = f"instances/{application.region.region_name}_{application.router.model_name}_{application.application_id}"
    ...
    def memory(self, memory):
        ...
        # if (memory - self.model_memory) / self.max_available_memory > 0.5:
        # print(f"Updating memory at {self.application.region} - {self.model} to: {memory}, fraction of available used: {(memory - self.model_memory) / self.max_available_memory}")
...
class ORCAInstance(Instance):
    ...
    def __init__(self,
        ...
        self.num_prompt_tasks_in_batch = 0
        self.num_token_tasks_in_batch = 0

        ## token-level tracking metadata
        self.pending_tokens = 0
        self.batch_tokens = 0
        self.cnt_iw_requests = 0
        ...
        # deprioritize dev requests by 0.8 * sla
        self.pending_requests = SortedList(key=lambda x: (x.arrival_timestamp + (4 * x.sla // 5)) if x.workload_type == 'dev' else x.arrival_timestamp)
    ...
    def log_iteration(self):
        ...
        try:
            self.scheduler_logger.debug("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s",
                                        ...
                                        iteration_end,
                                        len(self.batch),
                                        self.num_prompt_tasks_in_batch,
                                        self.memory,
                                        len(self.pending_requests),
                                        len(self.pending_queue),
                                        len(self.blocked_queue),
                                        len(self.pending_prompt_queue),
                                        self.pending_requests[0].request_id if len(self.pending_requests) > 0 else "None",
    ...
    def add_to_pool(self, task):
        ...
        if task.request not in self.pending_requests:
            self.pending_requests.add(task.request)
            self.request_tasks[task.request] = SortedList([task], key=lambda x: x.get_id())
        else:
            self.request_tasks[task.request].add(task)
    ...
    def task_arrival(self, task):
        task.instance = self
        task.arrive()
        if task.request.workload_type == "prod":
            self.cnt_iw_requests += 1
    ...
    def add_to_batch(self, task):
        ...
        if isinstance(task, PromptTask):
            self.num_prompt_tasks_in_batch += 1
        elif isinstance(task, TokenTask):
            self.num_token_tasks_in_batch += 1
        else:
    ...
    def get_num_contiguous_iterations(self):
        ...
        if len(self.batch) == 0:
            return 0
        if self.num_prompt_tasks_in_batch > 0:
            return 1
    ...
    def start_iteration(self):
        ...
        # NOTE: This was the stupidest piece of code to exist in this place. 
        # for request in self.pending_requests.values():
        #     task = self.request_tasks[request.request_id][0]
        ...
        # logging.info(f"Execution time at {self.application.region.region_name} - {self.model.name}: {self.iteration_duration}")
        self.completion_events["iteration"] = schedule_event(
                        self.iteration_duration * self.num_contiguous_iterations,
                        lambda instance=self: instance.complete_iteration())
        if self.completion_events["iteration"] == None:
            print("none!!!")
            exit()
    ...
    def pause_iteration(self):
        ...
        """
        if self.pause_next_iteration or self.num_prompt_tasks_in_batch > 0:
            return
...
class SplitwiseInstance(ORCAInstance):
    ...
    def task_arrival(self, task):
        ...
        if isinstance(task, PromptTask):
            batch_prompt_tokens = self.batch_tokens - self.num_token_tasks_in_batch
            if self.num_prompt_tasks_in_batch < len(self.batch) and \
