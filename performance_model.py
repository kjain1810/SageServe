    ...
    def get_iteration_duration(self,
        ...
        if len(prompt_tasks) == len(batch):
            ...
            if iteration_time is None:
                ...
                self.prompt_time_cache[cache_key] = float(iteration_time)
            iteration_time *= 1.1
        iteration_time = abs(iteration_time) + 0.01 # TODO: fix hack
        assert iteration_time > 0, f"Iteration time can not be {iteration_time}"
        print(f"{cache_key}: {iteration_time}")
