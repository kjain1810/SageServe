...
from simulator import clock, inc_fed_count, schedule_event, cancel_event, reschedule_event
...
class RequestState(IntEnum):
    ...
    """
    NONE = 0
    QUEUED_AT_GLOBAL_ROUTER = 1
    QUEUED_AT_REGION_ROUTER = 2
    QUEUED_AT_MODEL_ENDPOINT_ROUTER = 3
    QUEUED_AT_APPLICATION_SCHEDULER = 4
    RUNNING_ON_EXECUTOR = 5
    COMPLETED_AT_APPLICATION_SCHEDULER = 6
    COMPLETED_AT_MODEL_ENDPOINT_ROUTER = 7
    COMPLETED_AT_REGION_ROUTER = 8
    COMPLETED_AT_GLOBAL_ROUTER = 9
    ABORTED = 10
...
class WorkloadType(IntEnum):
    """
    WorkloadType describes the types of workload. PROD workloads are prioritized over DEV workloads.
    """
    DEV = 0
    PROD = 1
...
class Region(IntEnum):
    """
    Regions describe where the request is to be served.
    """
    WESTUS = 0
    CENTRALUS = 1
    EASTUS = 2
...
class Request():
    ...
    """
    # request_id,client_tenant,request_type,scenario,sla,regions,model_type,workload_type,application_id,arrival_timestamp,batch_size,prompt_size,token_size
    request_id: int
    node_id: count = field(default_factory=count)
    batch_id: int
    client_tenant: Region
    request_type: RequestType
    scenario: str
    sla: int
    utility: float
    regions: str
    region_priority: list = field(default_factory=list)
    model_type: str
    workload_type: str
    application_id: int
    batch_size: int = 1
    ...
    def __post_init__(self):
        for region in self.regions:
            self.region_priority.append(Region(int(region)))
    ...
    def arrive_at_global_router(self):
        assert self.state == RequestState.NONE
        self.metrics.global_router_arrival_timestamp = clock()
        self.state = RequestState.QUEUED_AT_GLOBAL_ROUTER
    
    def arrive_at_region_router(self):
        assert self.state == RequestState.QUEUED_AT_GLOBAL_ROUTER
        self.metrics.region_router_arrival_timestamp = clock()
        self.metrics.global_router_queue_time = clock() - \
                                    self.metrics.global_router_arrival_timestamp
        self.state = RequestState.QUEUED_AT_REGION_ROUTER

    def arrive_at_model_endpoint_router(self):
        assert self.state == RequestState.QUEUED_AT_REGION_ROUTER
        self.metrics.model_endpoint_router_arrival_timestamp = clock()
        self.metrics.region_router_queue_time = clock() - \
                                    self.metrics.region_router_arrival_timestamp
        self.state = RequestState.QUEUED_AT_MODEL_ENDPOINT_ROUTER
    
    def arrive_at_application_scheduler(self):
        if self.workload_type == 'dev':
            inc_fed_count()
        assert self.state == RequestState.QUEUED_AT_MODEL_ENDPOINT_ROUTER
        self.metrics.application_scheduler_arrival_timestamp = clock()
        self.metrics.model_endpoint_router_queue_time = clock() - \
                                    self.metrics.model_endpoint_router_arrival_timestamp
        self.state = RequestState.QUEUED_AT_APPLICATION_SCHEDULER

    def run_on_executor(self):
        assert self.state == RequestState.QUEUED_AT_APPLICATION_SCHEDULER
        self.metrics.executor_start_timestamp = clock()
        self.metrics.application_scheduler_queue_time = clock() - \
                                    self.metrics.application_scheduler_arrival_timestamp
        self.state = RequestState.RUNNING_ON_EXECUTOR

    def complete_at_application_scheduler(self):
        """
        NOTE: we don't track executor <--> scheduler communication overheads
        """
        assert self.state == RequestState.RUNNING_ON_EXECUTOR
        self.metrics.application_scheduler_completion_timestamp = clock()
        self.metrics.service_time += clock() - \
                                    self.metrics.executor_start_timestamp
        self.metrics.application_scheduler_response_time = clock() - \
                                    self.metrics.application_scheduler_arrival_timestamp
        self.state = RequestState.COMPLETED_AT_APPLICATION_SCHEDULER
    
    def complete_at_model_endpoint_router(self):
        """
        NOTE: we don't track scheduler <--> router communication overheads
        """
        assert self.state == RequestState.COMPLETED_AT_APPLICATION_SCHEDULER
        self.metrics.model_endpoint_router_completion_timestamp = clock()
        self.metrics.model_endpoint_router_response_time = clock() - \
                                    self.metrics.model_endpoint_router_arrival_timestamp
        self.state = RequestState.COMPLETED_AT_MODEL_ENDPOINT_ROUTER

    def complete_at_region_router(self):
        """
        NOTE: we don't track scheduler <--> router communication overheads
        """
        assert self.state == RequestState.COMPLETED_AT_MODEL_ENDPOINT_ROUTER
        self.metrics.region_router_completion_timestamp = clock()
        self.metrics.region_router_response_time = clock() - \
                                    self.metrics.region_router_arrival_timestamp
        self.state = RequestState.COMPLETED_AT_REGION_ROUTER
    
    def complete_at_global_router(self):
        """
        NOTE: we don't track scheduler <--> router communication overheads
        """
        assert self.state == RequestState.COMPLETED_AT_REGION_ROUTER
        self.metrics.global_router_completion_timestamp = clock()
        self.metrics.global_router_response_time = clock() - \
                                    self.metrics.global_router_arrival_timestamp
        self.state = RequestState.COMPLETED_AT_GLOBAL_ROUTER
    ...
    def abort(self):
        if self.state == RequestState.QUEUED_AT_GLOBAL_ROUTER:
            self.metrics.global_router_queue_time += clock() - \
                                    self.metrics.global_router_arrival_timestamp
        elif self.state == RequestState.QUEUED_AT_REGION_ROUTER:
            self.metrics.region_router_queue_time += clock() - \
                                    self.metrics.region_router_arrival_timestamp
        elif self.state == RequestState.QUEUED_AT_MODEL_ENDPOINT_ROUTER:
            self.metrics.model_endpoint_router_queue_time += clock() - \
                                    self.metrics.model_endpoint_router_arrival_timestamp
        elif self.state == RequestState.QUEUED_AT_APPLICATION_SCHEDULER:
            self.metrics.application_scheduler_queue_time += clock() - \
                                    self.metrics.application_scheduler_arrival_timestamp
        elif self.state == RequestState.RUNNING_ON_EXECUTOR:
            self.metrics.service_time += clock() - \
                                    self.metrics.executor_start_timestamp
        elif self.state == RequestState.COMPLETED_AT_APPLICATION_SCHEDULER:
            pass
        elif self.state == RequestState.COMPLETED_AT_APPLICATION_SCHEDULER:
            pass
        elif self.state == RequestState.COMPLETED_AT_MODEL_ENDPOINT_ROUTER:
            pass
        elif self.state == RequestState.COMPLETED_AT_REGION_ROUTER:
            pass
        elif self.state == RequestState.COMPLETED_AT_GLOBAL_ROUTER:
            pass
...
class GenerativeLLMRequest(Request):
    ...
    def generated_tokens(self, generated_tokens):
        ...
        if generated_tokens > self.max_seq_len:
            ...
            logging.info("Maximum sequence length exceeded")
