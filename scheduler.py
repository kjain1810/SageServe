...
from simulator import clock, feed_async, schedule_event, cancel_event, reschedule_event
...
class Scheduler(ABC):
    ...
    def __init__(self,
        ...
        # request queues
        self.pending_queue_length = 0
        self.array_results = {}
        self.reset_array_results()
        self.node_metrics = []
        # executors
        ...
        logger_name = f"schedulers/{application.region.region_name}_{application.router.model_name}_{application.application_id}"
        level = logging.INFO
    ...
    def reset_array_results(self):
        self.array_results["response_times"] = []
        self.array_results["queue_times"] = []
        self.array_results["ttft_times"] = []
        self.array_results["tbt_times"] = []
        self.array_results["nth_token_overheads"] = []
        self.array_results["prompt_sizes"] = []
        self.array_results["token_sizes"] = []
    ...
    def request_arrival(self, request):
        ...
        """
        request.arrive_at_application_scheduler()
        self.pending_queue_length += 1
    ...
    def save_all_request_metrics_intermediate(self, write_iter):
        node_metrics_df = pd.DataFrame(self.node_metrics)
        os.makedirs("request_nodes", exist_ok=True)
        application_identifier = f"{self.application.region.region_name}_{self.application.router.model_name}_{self.application.application_id}"
        utils.save_df_to_csv(node_metrics_df, f"request_nodes/{application_identifier}/{write_iter}.csv")
        self.last_log_iter = write_iter
        self.node_metrics = []
    ...
    def save_all_request_metrics(self):
        ...
        os.makedirs("request_nodes", exist_ok=True)
        application_identifier = f"{self.application.region.region_name}_{self.application.router.model_name}_{self.application.application_id}"
        utils.save_df_to_csv(node_metrics_df, f"request_nodes/{application_identifier}/{self.last_log_iter + 1}.csv")

    def get_results_intermediate(self, write_itr):
        array_results = self.get_results()
...
class RoundRobinScheduler(Scheduler):
    ...
    def schedule(self, request, *args, **kwargs):
        ...
        if self.instance_index >= len(self.instances):
            self.instance_index = 0

        instance = self.instances[self.instance_index]
        # logging.info("Scheduled on %s %s %s %s", self.application.region.region_name, self.application.router.model_name, self.application.application_id, self.instance_index)
