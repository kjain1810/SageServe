...
class Link():
    ...
    # queues
    pending_queue = SortedList(key=lambda x: x.get_id())

    executing_queue_len: int = 0
    ...
    def flow_arrival(self, flow):
        ...
        flow.instance = self
        flow.arrive()
        self.pending_queue.add(flow)
        if len(self.pending_queue) > 0 and self.executing_queue_len < self.max_flows:
    ...
    def flow_completion(self, flow):
        ...
        if len(self.pending_queue) > 0 and self.executing_queue_len < self.max_flows:
            ...
            if next_flow.dest.memory + next_flow.request.memory <= next_flow.dest.max_memory:
                ...
                # will lead to OOM
                self.run_flow(next_flow)
        flow.executor.remove_request()

    def retry_flow(self, flow):
        """
        Flow is retried on this Link.
        """
        if self.pending_queue.count(flow) == 0:
            return
        if (self.executing_queue_len < self.max_flows) and (flow.dest.memory + flow.request.memory <= flow.dest.max_memory):
            self.run_flow(flow)
        elif self.retry:
            schedule_event(self.retry_delay, lambda link=self,flow=flow: link.retry_flow(flow))
        else:
